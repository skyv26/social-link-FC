...
export const UserCard = (user: PersonDetailWithoutSocialLinks) => {
    ...
    userName.className = 'text-white font-inter font-bold text-2xl';
    const userPlace = document.createElement('h2');
    userPlace.textContent = user.based_in;
    userPlace.className = 'text-green font-inter font-bold text-xs min-[375px]:text-sm';

    const userDescription = document.createElement('p');
    userDescription.textContent = `"${user.description}"`;
    userDescription.className = 'text-white font-inter text-[10.5px] min-[375px]:text-[14px]';
